...
b2DynamicTree *b2DynamicTree::Clone()
{
  b2DynamicTree *cloned = new b2DynamicTree();
  b2Free(cloned->m_nodes);

  cloned->m_nodeCapacity = m_nodeCapacity;
  cloned->m_nodeCount = m_nodeCount;
  cloned->m_nodes = (b2DynamicTreeNode*)b2Alloc(m_nodeCapacity * sizeof(b2DynamicTreeNode));
  memcpy(cloned->m_nodes, m_nodes, m_nodeCapacity * sizeof(b2DynamicTreeNode));

  cloned->m_root = m_root;
  cloned->m_path = m_path;

  cloned->m_freeList = m_freeList;
  cloned->m_insertionCount = m_insertionCount;
